"""Build the single-file Kaggle script for Atom language transfer v1."""
...
SOURCE_FILES = (
    ...
    PROJECT_ROOT / "atom_language_transfer_dataset.py",
    PROJECT_ROOT / "atom_language_field.py",
    PROJECT_ROOT / "atom_language_side_view.py",
    PROJECT_ROOT / "atom_language_transfer_experiment.py",
)
...
def build_bundle(output_dir: Path) -> dict[str, object]:
    ...
    bundle_path = output_dir / "atom_language_transfer_kaggle.py"
    sections = [
        strip_module_declarations(
            path,
            drop_definitions=(
    ...
    bundle = (
        '"""Generated Kaggle bundle for Atom disjoint-lexicon transfer v1."""\n\n'
        "from __future__ import annotations\n\n"
        + BUNDLE_IMPORTS
        + "\n"
        + "\n\n".join(sections)
    ...
    metadata = {
        "id": "jessealicea/atom-language-disjoint-transfer-v1",
        "title": "Atom Language Disjoint Transfer v1",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "kaggle" / "language-transfer-v1",
    )
